...
export const command: Command = {
    ...
    async execute(client, interaction) {
		...
		var userData1: MemberResponse = (await fetch(env.URL + "/api/guilds/" + interaction.guildId + "/users/" + user1).then(res => res.json()));
		var userData2: MemberResponse = (await fetch(env.URL + "/api/guilds/" + interaction.guildId + "/users/" + user2).then(res => res.json()));
