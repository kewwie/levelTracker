...
export const command: Command = {
    ...
    async execute(client, interaction) {
        ...
        var data: MemberResponse = await fetch(env.URL + "/api/guilds/" + interaction.guildId + "/users/" + user).then(res => res.json());
        ...
        if (wantedLevel) {
            ...
            var { xp } = (await fetch(
                env.URL + `/api/level?` + new URLSearchParams({ level: String(wantedLevel) })
            ).then(res => res.json())); 
